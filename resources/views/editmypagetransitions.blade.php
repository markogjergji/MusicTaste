<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <script src="https://kit.fontawesome.com/f53ffe0a63.js" crossorigin="anonymous"></script>
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@600&display=swap" rel="stylesheet">
    <link href="//db.onlinewebfonts.com/c/4a373e70cd01dec089c1dd1185ceba51?family=Maison+Neue" rel="stylesheet" type="text/css"/>
    
    <link href="{{ asset('css/editTransitions.css') }}" rel="stylesheet">
    <script src="{{ url('js/addTransitions.js') }}"></script>


    <title>Document</title>
</head>
<body>
    

    <div id="main-page-container">
    <div id="welcome-title">
        <div id="welcome-text-con">
            <input type = "text" value="Welcome to my wall" id="welcome-text" maxlength="70" />
        </div>
    </div>
        
    </div>
</body>
</html>